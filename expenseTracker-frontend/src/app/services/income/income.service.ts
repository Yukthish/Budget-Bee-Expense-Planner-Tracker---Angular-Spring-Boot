...
//       return this.http.get(BASIC_URL + `app/income/${id}`);
//     }
...
export class IncomeService {
  ...
  getIncomeById(id: number): Observable<any> {
    return this.http.get(BASIC_URL + `api/income/${id}`);  // Updated path
  }

  updateIncome(id: number, incomeDTO:any): Observable<any> {
    return this.http.put(BASIC_URL + `api/income/${id}`,incomeDTO);  // Updated path
  }
